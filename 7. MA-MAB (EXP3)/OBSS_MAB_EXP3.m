...
ideal_capacity = Capacity(20e6,db2pow(100));
...
for r=1:R % We repeat the experiment during 'R' repetitions for taking the average
    ...
    weights_per_arm = ones(N_WLANs,size(states_wlan,2));
    p_per_arm = zeros(N_WLANs,size(states_wlan,2));
    ...
    while(iteration <= N)        
        ...
        for i=1:N_WLANs
            % Select best arm
            for j=1:size(states_wlan,2)
                p_per_arm(i,j)=(1-gamma)*(weights_per_arm(i,j)/sum(weights_per_arm(i,:))) + gamma/size(states_wlan,2);
            end            
            selected_arm(i) = randsample(1:size(states_wlan,2), 1, true, p_per_arm(i,:));
            %selected_arm(i) = ChooseArm(p_per_arm(i,:));
        ...
        reward = mean(tpt_aux)/ideal_capacity;
        progressive_tpt(r,iteration) = mean(tpt_aux);
        progressive_fairness(r,iteration) = JainsFness(tpt_aux);
        % Assign reward to configurations set at each WLAN
        for i=1:N_WLANs
            estimated_reward = reward / p_per_arm(i,selected_arm(i));
            weights_per_arm(i,selected_arm(i)) = weights_per_arm(i,selected_arm(i))*exp(gamma*estimated_reward/size(states_wlan,2));
...
if R == 1
    ...
    figure
    progressive_tpt_smooth = smooth(progressive_tpt);
    plot(0:T:(size(progressive_tpt,2)*100)-1, mean(progressive_tpt_smooth));
    xlabel('Number of iterations');
    ylabel('Average Throughput (bps)');
    title('Evolution of the throughput');

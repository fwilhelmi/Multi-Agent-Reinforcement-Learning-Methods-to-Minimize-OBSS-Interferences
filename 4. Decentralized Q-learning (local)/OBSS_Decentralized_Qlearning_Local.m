...
% Decentralized Q-learning - LOCAL Approach
...
disp('DECENTRALIZED Q-LEARNING - LOCAL APPROACH')
disp('-----------------------')
...
for r=1:R % We repeat the experiment during 'R' repetitions for taking the average 
    ...
    while(iteration <= N)
        ...
        for i=1:N_WLANs
            reward(i,iteration) = throughput(i,iteration);
            Qval{i}(curr_state(i))=(1-alpha)*Qval{i}(curr_state(i)) + alpha*(reward(i,iteration)+gamma*Qval{i}(next_state(i))); %Update Q
        end

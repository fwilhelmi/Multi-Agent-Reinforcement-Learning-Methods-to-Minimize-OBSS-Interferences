...
% Multi-Agent Multi-Armed Bandits Problem - Epsilon-Greedy
...
disp('MA-MAB APPROACH - EPSILON-GREEDY')
disp('-----------------------')
...
for r=1:R % We repeat the experiment during 'R' repetitions for taking the average
    ...
    while(iteration <= N)        
        ...
        for i=1:N_WLANs
            reward_per_configuration(i,selected_arm(i)) = mean([reward_per_configuration(i,selected_arm(i)) reward]);
